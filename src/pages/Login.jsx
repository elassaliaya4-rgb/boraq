...
export default function Login({ isModal = false }) {
      ...
      if (cleanCode === "admin" || cleanCode === "boraq") {
        ...
        if (ag && ag.email) {
          ...
          if (drv && drv.email) {
            ...
            if (testErr) {
              const tempClient = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY, {
  ...
  const formContent = (
    <div className="login-card" style={isModal ? { background: "none", border: "none", boxShadow: "none", padding: 0 } : {}}>
      ...
      {loginMode === "code" ? (
        <div className="field">
          <label>{lang === "ar" ? "كود الوكالة أو السائق" : "Code de l'agence ou chauffeur"}</label>
          <input
            type="text"
            value={code}
            onChange={(e) => setCode(e.target.value)}
            placeholder="AG-001 / Driver Code"
            autoCapitalize="none"
            autoCorrect="off"
            spellCheck="false"
